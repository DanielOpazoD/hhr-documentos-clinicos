...
test("keeps the clinical studios usable from mobile through desktop", async () => {
  ...
  assert.match(documentStudio, /aria-label="No se pudo cargar la plantilla\. Reintentar"/);
  assert.match(documentStudio, /onClick=\{retryTemplateSettings\}/);
  assert.doesNotMatch(documentStudio, /templateSettingsError \? retryTemplateSettings\(\)/);
  ...
  assert.match(documentStudio, /aria-controls="document-print-preflight"/);
  assert.match(documentStudio, /aria-expanded=\{preflightOpen\}/);
  assert.match(documentStudio, /disabled=\{printingDisabled\}/);
  assert.match(documentStudio, /role="status" aria-live="polite"/);
  assert.match(documentStudio, /aria-label="Cerrar revisión de impresión"/);
  ...
  assert.match(commandBarSource, /Cambios sin guardar/);
  assert.match(commandBarSource, /const saveState = saving[\s\S]*\? "Guardando…"[\s\S]*: saveError/);
  assert.match(commandBarSource, /Guardado \$\{savedAt\}/);
  assert.match(commandBarSource, /Error al guardar/);
  assert.match(commandBarSource, /role="status"/);
  ...
  assert.doesNotMatch(previewSource, /status-pill|\{status\} · v\{version\}/);
  assert.match(previewSource, /No se pudo cargar la plantilla/);
  ...
  assert.doesNotMatch(responsiveStyles, /\.patient-editor\.collapsed|\.identity-collapse-trigger/);
  assert.doesNotMatch(responsiveStyles, /\.paper-toolbar\s*\{[^}]*position:\s*sticky/);
  assert.doesNotMatch(globalStyles, /@import\s+["']tailwindcss["']/);
  assert.match(styles, /\.studio-page \.document-command-bar \{[^}]*display: flex;[^}]*align-items: center;/);
...
test("contains no production sample workflow or fictitious record creation", async () => {
  ...
  assert.match(headersConfig, /Referrer-Policy", value: "strict-origin-when-cross-origin"/);
  assert.match(headersConfig, /source: "\/captura"[\s\S]*Referrer-Policy", value: "no-referrer"/);
  assert.ok(headersConfig.indexOf('source: "/captura"') < headersConfig.indexOf('source: "/:path*"'));
});
