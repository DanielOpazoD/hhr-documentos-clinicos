...
test("keeps the clinical studios usable from mobile through desktop", async () => {
  ...
  assert.match(commandBarSource, /Guardando…/);
  ...
  assert.match(commandBarSource, /const saveConflict = saveError\.includes\("otra pestaña"\)/);
  assert.match(commandBarSource, /\{!saveConflict \? \([\s\S]*Reintentar guardado/);
