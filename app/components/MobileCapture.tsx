...
export function MobileCapture({ token }: { token: string }) {
  ...
  if (access === "error") return <main className="capture-shell"><div className="capture-status error"><OperationFeedback tone="error" title="No se pudo verificar la sesión" message={accessError?.message ?? "Revise su conexión e intente nuevamente."} supportId={accessError?.supportId} actions={accessError?.retryable ? <button className="text-button" onClick={() => { setAccess("checking"); setAccessError(null); setValidationAttempt(value => value + 1); }}>Reintentar</button> : null} /></div></main>;
  ...
  return <main className="capture-shell"><header className="capture-header"><img src="/hhr-logo.svg" alt="Hospital Hanga Roa" /><div><strong>Escáner HHR</strong><small>Sesión temporal · {expiresAt && `hasta ${new Date(expiresAt).toLocaleTimeString("es-CL", { hour: "2-digit", minute: "2-digit" })}`}</small></div><span className="capture-page-count">{pages.length}/{pageLimit}</span></header>
    <section className="capture-content"><div className="capture-intro"><span className="eyebrow">Documento multipágina</span><h1>Escanee con la cámara</h1><p>Capture, ajuste los bordes y elija el estilo de cada página.</p></div>
      ...
      {error ? <OperationFeedback compact tone="error" title="No se pudo completar el escaneo" message={error.message} supportId={error.supportId} onDismiss={() => setError(null)} className="capture-error" /> : null}
