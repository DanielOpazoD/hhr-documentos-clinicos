...
type PdfImageSource = { source: CanvasImageSource; width: number; height: number; release: () => void };
...
async function normalizedImage(file: File, rotation: number) {
  ...
  try {
    ...
    context.drawImage(image.source, -image.width * printScale / 2, -image.height * printScale / 2, image.width * printScale, image.height * printScale);
    return { dataUrl: canvas.toDataURL("image/jpeg", 0.995), width: canvas.width, height: canvas.height, format: "JPEG" };
...
export async function createScannedPdf(pages: Array<{ file: File; rotation: number }>) {
  ...
  for (let index = 0; index < pages.length; index++) {
    ...
    pdf.addImage(image.dataUrl, image.format, (pageWidth - width) / 2, (pageHeight - height) / 2, width, height, undefined, image.format === "PNG" ? "FAST" : "NONE");
  }
