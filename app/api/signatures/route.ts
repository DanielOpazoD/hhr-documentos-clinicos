...
export async function POST(request: Request) {
  ...
  await db.prepare(`INSERT INTO signatures (id, owner_email, kind, professional_name, professional_rut, specialty, object_key, mime_type, size, is_default, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?)`).bind(id, owner, kind, professionalName, professionalRut, specialty, objectKey, file.type, file.size, now, now).run();
