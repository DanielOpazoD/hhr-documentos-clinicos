...
export function useFilesLibrary() {
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : "No se pudieron eliminar los archivos.");
    } finally {
      setBusy(false);
    }
  ...
  async function toggleArchive(file: SavedFile) {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : "No se pudo actualizar el archivo.");
    }
  }
